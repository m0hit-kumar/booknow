...
User? user;
  ...
  Future<void> _bookAppointment(
      String doctorId, String dateStr, String time) async {
    try {
      DocumentReference docRef =
          _firestore.collection('appointments').doc(doctorId);
      ...
      if (slotIndex != -1) {
        ...
        dateSlots[slotIndex]['bufferTime'] =
            slotBuffers[dateStr]?[time] ?? bufferTime;
      } else {
                  ...
                  return Padding(
                    ...
                    child: GridView.builder(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        final bufferTime =
                            slot['bufferTime'] ?? this.bufferTime;
                        final isBooked = slot['isBooked'] ?? false;
...
// Determine slot state and color
                        ...
                        return GestureDetector(
                          ...
                          //     ? null
                          //     : () => _setBufferTime(dateStr, time),
                          child: Container(
                            decoration: BoxDecoration(
                              color: isBookedSlot
                                  ? Colors.grey.withValues(
                                      alpha: 0.1) // Grey when booked
                                  : isUnavailableSlot
                                      ? Colors.red.withValues(
                                          alpha: 0.1) // Red when unavailable
                                      : Colors.green.withValues(
                                          alpha: 0.1), // Green when available
                              border: Border.all(
                                color: isBookedSlot
                                    ? Colors.grey
                                    : isUnavailableSlot
